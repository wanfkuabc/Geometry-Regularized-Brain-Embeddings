    ...
    def __init__(self,  z_dim, c_num, timesteps, embedding_dim = 1440):
        ...
        self.project = nn.Sequential(
            ...
            ResidualAdd(
                nn.Sequential(
                # nn.GELU(),
                nn.Linear(z_dim, self.hidden_dim, bias=False),
                nn.Dropout(0.3),
                nn.Linear(self.hidden_dim, z_dim, bias=False),
                )
                ),
            nn.LayerNorm(z_dim, eps=1e-6))
        self.logit_scale = nn.Parameter(torch.ones([]) * np.log(1 / 0.07))
        self.softplus = nn.Softplus()
...
# class EEGMLPProjectModel(nn.Module):
#     """
#     你的创新模型：
#     直接对 EEG [B,C,T] flatten 后做投影，不依赖卷积 backbone
#     """
#     def __init__(self, z_dim, c_num, timesteps, dropout=0.3):
#         super().__init__()
#         self.z_dim = z_dim
#         self.c_num = c_num
#         self.timesteps = timesteps
