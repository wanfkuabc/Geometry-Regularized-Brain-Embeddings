...
class EEGDataset(Dataset):
    def __init__(self, config, mode):
        ...
        if  os.path.exists(features_filename):
            ...
            self.vlmodel, self.preprocess,_ = open_clip.create_model_and_transforms(self.model_type, device="cuda:0",pretrained=pretrain_map[self.model_type]['pretrained'])
    ...
    def __getitem__(self, index):
        ...
        sample  = {
            ...
            # 新增，给 adapter 用
            'subject_id': torch.tensor(subject, dtype=torch.long),

            # 可选，调试/日志/可视化时很好用
            'subject_name': self.subjects[subject],
